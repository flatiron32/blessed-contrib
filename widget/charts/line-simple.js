...
function Line(options) {
  ...
  if (options.callback) {
    if (options.immediate) {
      setImmediate(options.callback(self));
    }
...
Line.prototype.setData = function(labels, data) {
  ...
  if (!this.ctx) {
    console
        .log("error: canvas context does not exist. setData() for line charts must be called after the chart has been added to the screen via screen.append()");
    return;
  }
  ...
  function getYPixel(val) {
    ...
    res-- // to separate the baseline and the data line to separate chars so
    // canvas will show separate colors
    return res
  }
